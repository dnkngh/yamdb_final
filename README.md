...
DB_NAME=postgres
POSTGRES_USER=<username для postgres (без скобок)>
POSTGRES_PASSWORD=<пароль для postgres (без скобок)>
DB_HOST=db
DB_PORT=5432
```

##### 4. Выполните docker-compose
Выполните из директории `./infra` команду:
```
docker-compose up -d
```

##### 5. Создайте и выполните миграции, подгрузите статику
```
...
docker-compose down
```
Повторная сборка и запуск контейнеров:
```
docker-compose up -d --build
```

##### 8. Заполнение базы данных
Базу данных можно заполнить тестовыми данными, выполнив команду
```
